...
class Navi:
    ...
    def __get_navi_et_serialnum_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/et_serialnum')

    # 获取激活码控件
    def __get_navi_et_activenum_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/et_activenum')

    # 获取导航激活确定控件
    def __get_navi_active_confirmorcancel(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/tv_active_confirmorcancel')

    # 获取当前道路名称控件
    def get_navi_current_road_name_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/ll_current_name')

    # 获取指南针控件
    def get_navi_Compass_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/llCompass')
    ...
    def chk_navi_is_home(self):
        ...
        if not (ele.wait.exists() or ele1.wait.exists()):
            uit.raise_Exception_info('当前不是导航主界面')
    ...
    def click_navi_home_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('返回主界面控件不存在')
    ...
    def click_navi_current_road_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('当前道路控件不存在')
    ...
    def get_navi_search_key_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/search_key')
    ...
    def get_navi_search_addr_list_title(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/list_item_title')

    # 获取查询结果列表list视图
    def get_navi_search_listview_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/searchtip')

    # 获取准备导航的目的地地址
    def get_navi_ready_dest_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/tv_name')

    # 获取地址收藏控件
    def get_navi_ready_dest_fav_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/iv_save')

    # 获取导航到目的地控件
    def get_navi_ready_to_dest_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/iv_dest')

    # 获取导航界面的导航指示图
    def get_navi_navipager_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/pager')

    # 获取导航界面的实时路况控件
    def get_navi_time_indicator_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/fl_tmc_indicator')

    #获取导航搜索界面的收藏控件
    def get_navi_search_fav_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/li_layout2')

    #获取附近的XXX 的标题
    def get_navi_nearby_title_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/tv_title')
    ...
    def get_navi_nearby_back_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/btn_back')

    # 获取查询结果条目控件
    def __get_navi_search_result_all_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/ll_result_all')
    ...
    def get_navi_his_fav_del_ele(self, addr):
        ...
        if result_ele.wait.exists():
            ...
            uit.raise_Exception_info('列表控件不存在或者列表为空')
    # 获取返回地图控件mapback
    def __get_navi_mapback_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/iv_mapback')

    # 获取关闭附近的控件
    def __get_navi_nearby_close_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/btn_close')
    ...
    def active_navi(self):
        ...
        if confirmorcancel_ele.wait.exists(timeout=ht.LONG_TIME_OUT):
            time.sleep(5)
