...
class Audio:
    ...
    def __get_audio_list_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/control_name')
    ...
    def __get_audio_mine_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':drawable/menu_selector_mine')

    # 获取酷我音乐控件
    def __get_audio_kuwo_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':drawable/menu_selector_baidu')

    # 获取返回主界面控件
    def __get_audio_home_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':drawable/menu_selector_home')
    #获取今日歌单控件
    def __get_audio_smart_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':drawable/menu_selector_smart')
    ...
    def __get_audio_mine_bluetooth_play_ele(self):
        ele = uit.get_ele_by_resourceId(pkg_name + ':drawable/menu_selector_bluetooth')
        return ele
    ...
    def __get_audio_search_listview_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/search_listview')
    # 获取音乐搜索列表中的音乐名
    ...
    def __get_audio_back_to_radio_ele(self):
        return uit.get_ele_by_resourceId(pkg_name + ':id/radio_back_btn')
    ...
    def click_pause_or_play_ele(self):
        ...
        if next_ele.exists and prev_ele.wait.exists():
            ...
            uit.raise_Exception_info('上一首或者下一首控件不存在')
    ...
    def get_audio_name_playing_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐名字控件不存在')
    ...
    def get_audio_artist_playing_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐演唱者控件不存在')
    ...
    def get_audio_showtime_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐播放时间控件不存在')
    ...
    def get_audio_alltime_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐播放总时间控件不存在')
    ...
    def click_audio_mine_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('我的音乐库控件不存在')
    ...
    def click_audio_kuwo_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('酷我音乐控件不存在')
    ...
    def click_audio_home_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐的Home控件不存在')
    # 点击今日歌单控件
    def click_audio_smart_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('今日歌单控件不存在')
    ...
    def click_audio_mine_my_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('我的音乐控件不存在')
    ...
    def click_audio_mine_all_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('本地全部歌曲控件不存在')
    ...
    def get_audio_mine_usb_cnt_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('USB音乐条数控件不存在')
    ...
    def click_audio_mine_usb_play_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('USB音乐播放控件不存在')
    # 点击音乐收藏控件
    def click_audio_fav_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐收藏控件不存在')
    ...
    def click_audio_list_ta_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('听Ta的歌控件不存在')
    # 点击相似歌曲
    def click_audio_list_similar_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('相似歌曲控件不存在')
    ...
    def click_audio_kuwo_name_random_ele(self):
        ...
        if len(ele) > 0:
            ...
            uit.raise_Exception_info('酷我专辑为空')
            '''
    # 随机播放今日歌单音乐
    def click_audio_smart_name_random_ele(self):
        ...
        if len(ele) > 0:
            ...
            ht.set_context_map(name_txt, artist_txt)

            ele[idx].click.wait()
            # 返回音乐名称
            return name_txt
        else:
            uit.raise_Exception_info('今日歌单为空')
    ...
    def input_audio_search_keyword_ele(self, text):
        ...
        if ele.wait.exists():
            ele.set_text(ht.unicode_input(text))
        else:
    ...
    def click_audio_search_result_by_artist(self, artist):
        ...
        if music_search.wait.exists(timeout=ht.LONG_TIME_OUT):
            ...
            # 1s时间音乐播放界面刷新
                ...
                uit.raise_Exception_info('指定歌手名字不存在')
        else:
    ...
    def get_audio_search_keyword_txt(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('音乐搜索框控件不存在')
    ...
    def chk_audio_is_home_page(self):
        ...
        if not ele.wait.exists():
            uit.raise_Exception_info('当前界面不是音乐主界面')
    # 滚动listview
    def scroll_audio_list_ta_listview_forward(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('听ta的歌，列表控件不存在')
    # 根据歌手名字验证听ta的歌列表是否正确
    def chk_audio_list_ta_listview_by_artist(self, chk_artist):
        ...
        ele_artist.wait.exists(timeout=ht.LONG_TIME_OUT)
        if len(ele_artist) > 0:
            for e in ele_artist:
                # 判断不存在就报错
                if not str(e.text.strip()).__contains__(chk_artist):
                    uit.raise_Exception_info('听ta的歌列表校验失败，期望歌手为: ' + chk_artist + ', 实际歌手为: ' + e.text.strip())

        else:
            uit.raise_Exception_info('听ta的歌列表为空')
    # 从音乐界面返回到launcher界面
    ...
    def click_audio_mine_bluetooth_play_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('蓝牙音乐播放控件不存在')
    ...
    def click_audio_back_to_radio_ele(self):
        ...
        if ele.wait.exists():
            ...
            uit.raise_Exception_info('听歌识曲返回控件不存在')
    ...
    def back_from_kuwo(self):
        # package_name = d.info['currentPackageName']
        back = uit.get_ele_by_resourceId('cn.kuwo.kwmusiccar:id/layoutPlayControlPanel')
        if back.wait.exists():
            back.click.wait()
        else:
